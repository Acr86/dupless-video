...
| **audio_coverage** (`scan_audio_coverage`) | duration-scaled sparse probes (one per ~2 min, clamp [8,48]) × 5s windows, ffmpeg `volumedetect` over **all** audio streams (`-map 0:a?`); fraction of probes where any stream has `mean_volume > −60 dB` | float [0..1] | `n_points=None` → `coverage_points(duration)`, `win_s=5.0`, `silence_db=−60`; false-positive fixes: '↻ Re-check audio' (re-measure, overwrite cache) and 'Mark audio as OK' (`quality_overrides`, auto-expires when the file changes) |
